...
module.exports = function layoutshim (sails) {
  ...
  if (sails.config.views.layout) {
    ...
    if (sails.config.hooks.http) {
      ...
      else if (engineName === 'ect') {
        var ectLayoutEngine = require('ect');
        sails.log.verbose('Overriding handlebars engine with express-handlebars to implement layout support...');
        var ect = ectLayoutEngine({
          root: path.join('views', sails.config.views.partials || ''),
          ext: sails.config.views.extension || 'ect'
        })
        sails.config.views.engine.fn = ect.render;
      }
